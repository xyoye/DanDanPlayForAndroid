...
public class ShooterSubPresenterImpl extends BaseMvpPresenterImpl<ShooterSubView> implements ShooterSubPresenter {
    ...
    @Override
    public void downloadSubtitleFile(String fileName, String downloadLink) {
        ...
        getView().showLoading();
        ShooterSubDetailBean.downloadSubtitle(downloadLink, subtitleFile.getAbsolutePath(), new CommShooterDataObserver<Boolean>() {
            @Override
            public void onSuccess(Boolean downloadResult) {
                getView().hideLoading();
                if (downloadResult) {
                    getView().showError("文件下载成功：" + subtitleFile.getAbsolutePath());
                    getView().subtitleDownloadSuccess();
                } else {
                    getView().showError("文件保存失败");
                }
            }
