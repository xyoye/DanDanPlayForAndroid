...
public class TorrentService extends Service {
    ...
    public void onStartEvent(TorrentStartEvent event){
        ...
        //根据hash判断任务是否已经存在
        if (!TaskInfo.taskMap.containsKey(torrent.getHash())){
            TorrentTask torrentTask = new TorrentTask(torrent);
            torrentTask.startTask();
            showNotification();
        }
    }
    ...
    public void onEvent(TorrentEvent event){
        if (event.getPosition() >= TaskInfo.taskList.size()) {
        ...
        switch (event.getAction()){
            ...
            case TorrentEvent.EVENT_RESUME:
                TorrentTask resumeTask = TaskInfo.taskList.get(event.getPosition());
            ...
            case TorrentEvent.EVENT_PAUSE:
                TorrentTask pauseTask = TaskInfo.taskList.get(event.getPosition());
            ...
            case TorrentEvent.EVENT_ALL_PAUSE:
                for (TorrentTask task : TaskInfo.taskList){
                    if (!task.isPaused()){
                        task.pause();
                    }
                }
            ...
            case TorrentEvent.EVENT_DELETE_ALL_TASK:
                for (int i = 0; i< TaskInfo.taskList.size(); i++){
                    deleteTask(i, event.isDeleteFile());
                }
                break;
            case TorrentEvent.EVENT_PREPARE_PLAY:
                TorrentTask playTask = TaskInfo.taskList.get(event.getPosition());
    ...
    private Notification showNotification(){
        ...
        if (TaskInfo.taskList.size() == 0){
            downloadStatus = "无任务";
        }else if (doneTask == TaskInfo.taskList.size()){
            downloadStatus = "已完成";
        }else if (doingTask > 0){
            downloadStatus = "下载中";
        }else if (pauseTask == TaskInfo.taskList.size()){
            downloadStatus = "全部暂停";
        }else {
            downloadStatus = "未知";
        }
    ...
    private Notification buildNotification(String downloadStatus, int doneTask) {
        ...
        msg += " 进度："+doneTask+"/"+ TaskInfo.taskList.size();
    ...
    private void deleteTask(int position, boolean isDeleteFile){
        TorrentTask torrentTask = TaskInfo.taskList.get(position);
        //暂停任务
        if (torrentTask.isRunning() && !torrentTask.isPaused()){
            torrentTask.pause();
        }
        //从内存中移除数据
        TaskInfo.taskList.remove(position);
        TaskInfo.taskMap.remove(torrentTask.getTorrent().getHash());
        //新建线程操作数据库及文件
        IApplication.getExecutor().execute(() -> {
            //从数据库中移除任务
            TorrentUtil.deleteDBTorrent(torrentTask.getTorrent().getTorrentPath());
            //删除文件
            if (isDeleteFile){
                TorrentUtil.deleteTaskFile(torrentTask.getTorrent());
            }
        });
        //通知activity刷新界面
