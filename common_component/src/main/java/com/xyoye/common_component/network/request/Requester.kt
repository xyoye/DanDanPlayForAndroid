        ...
        scope.launch(context = Dispatchers.Main) {
            ...
            try {
                ...
                if (result is CommonJsonData && !result.success) {
                    onError?.invoke(RequestError.formJsonData(result))
                } else {
                ...
                onError?.invoke(RequestError.formException(e))
            } finally {
                onComplete?.invoke()
            }
        }
...
inline fun <T> httpRequest(scope: CoroutineScope, requester: Requester<T>.() -> Unit) {
    Requester<T>().apply(requester).doRequest(scope)
}