...
public class IJKSubtitleView extends View {
    //默认视频宽度
    ...
    //默认字幕第二行文字大小
    private static final float DEFAULT_SECOND_TEXT_SIZE = 35f;
    //默认字幕文字描边宽度
    ...
    public IJKSubtitleView(Context context) {
        this(context, null);
    }

    public IJKSubtitleView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public IJKSubtitleView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = mBottomSubtitles.length - 1; i >= 0; i--) {
            ...
            mTextSize = subtitle.size == 0
                    ? i == 0 ? DEFAULT_TEXT_SIZE : DEFAULT_SECOND_TEXT_SIZE
                    : subtitle.size;
